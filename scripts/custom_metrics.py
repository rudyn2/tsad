...
class _CustomBasePrecisionRecall(_BaseClassification):
    def __init__(
        self,
        ...
        self.eps = 1e-20
        super(_CustomBasePrecisionRecall, self).__init__(
            output_transform=output_transform, is_multilabel=is_multilabel, device=device
        )


class Recall(_CustomBasePrecisionRecall):
    ...
    @reinit__is_reduced
    def reset(self) -> None:
        self._true_positives = []  # type: Union[list, int, torch.Tensor]
        self._positives = []  # type: Union[list, int, torch.Tensor]
        super(_CustomBasePrecisionRecall, self).reset()

    def compute(self) -> Union[torch.Tensor, float]:
        if np.sum(self._positives) == 0:
            return 0

        result = np.sum(self._true_positives) / (np.sum(self._positives) + self.eps)
    ...
    def update(self, output: Sequence[torch.Tensor]) -> None:
        ...
        self._true_positives.append(true_positives)
        self._positives.append(actual_positives)
