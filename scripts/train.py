...
def train_for_classification(net, dataset, optimizer,
    ...
    for e in range(1, epochs + 1):
        ...
        for i, (x, s, tl, v_aff) in enumerate(train_loader):
            ...
            if use_wandb:
                wandb.log({'train/loss': float(avg_loss), 'train/acc TL': float(avg_tl_acc), 'train/acc SEG': float(avg_seg_acc), 'train/loss VA': float(avg_va_loss)}, step=global_step)
            global_step += 1

        tiempo_epochs += time.time() - inicio_epoch
        if use_wandb:
            wandb.log({'train/loss': float(avg_loss), 'train/acc TL': float(avg_tl_acc), 'train/acc SEG': float(avg_seg_acc), 'train/loss VA': float(avg_va_loss), 'epoch': e})
        ...
        if e % reports_every == 0:
            ...
            avg_tl_acc, avg_seg_acc, avg_va_loss = eval_net(device, net, val_loader, va_weights)
            ...
            if use_wandb:
                ...
                wandb.log({'val/acc TL': float(avg_tl_acc), 'val/acc SEG': float(avg_seg_acc), 'val/loss VA': float(avg_va_loss), 'epoch': e})
...
def eval_net(device, net, test_loader, va_weights):
    net.eval()
