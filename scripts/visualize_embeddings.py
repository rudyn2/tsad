...
if __name__ == "__main__":
    ...
    for i, (embeddings, embeddings_length, actions, speeds, embeddings_label) in enumerate(loader):
        embeddings, embeddings_label, actions, speeds = embeddings.to(args.device), embeddings_label.to(args.device), actions.to(
            args.device), speeds.to(args.device)
        pred_embedding = temp_model(embeddings, actions, speeds, embeddings_length)
        
        output = vis_model.decode(embeddings_label)
        segmentation = output['segmentation'].argmax(dim=1)

        output = vis_model.decode(pred_embedding)
        pred_segmentation = output['segmentation'].argmax(dim=1)
